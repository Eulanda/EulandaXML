...
The filenames of the XML files must follow a specific naming convention. 

The structure is **objectname-identifier-timestamp.xml where** all name components are to be used in **lowercase letters**. Besides the **letters** a-z only **digits** the **hyphen** and a **decimal point** are allowed.

### Object name

The following object names exist:

- **stock** for stock quantities of products
- **price** for price changes to products
...
The identifier is freely selectable and can also be omitted. To simplify retrieval, it is recommended to use the master key for individual data records. For example, if only one order is stored in the order\*.xml file, the order number should be used as the identifier; the same applies to status messages. If only one article is used per product\*.xml file, the article number or EAN number should be used as identifier. If there are several data sets in one file, the identifier can also be omitted.

### Timestamp

A sortable timestamp that allows the files to be ordered chronologically is ideal. e.g. yyyy-mm-dd-ss-mss. For example, a suitable file name for order 4711 might be: **order-4711-2021-09-15-12-16-236.xml**
